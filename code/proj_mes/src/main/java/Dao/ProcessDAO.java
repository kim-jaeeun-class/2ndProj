...
public class ProcessDAO {
    ...
    public boolean createProcess(ProcessDTO dto) {
        String sql = "INSERT INTO process (proc_name, process_check, dapart_id2, proc_img, proc_info, proc_seq, item_code, depart_level) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        int result = 0;
        try (Connection conn = getConn();
            ...
            pstmt.setString(8, dto.getDepart_level());
    ...
    public List<ProcessDTO> getProcessesBySearch(String itemCode, String departLevel, String procName) {
        ...
        // Use aliases (p and d) for clarity
        ...
        // Check if the parameter is for the department level
        ...
        try (Connection conn = getConn();
            ...
            int paramIndex = 1;
            // The order of setting parameters must match the order in the SQL query
            if (departLevel != null && !departLevel.isEmpty()) {
    ...
    public List<String> getUniqueItemCodes() {
        ...
        String sql = "SELECT DISTINCT item_code"
        		   + " FROM process WHERE item_code IS NOT NULL ORDER BY item_code";
        try (Connection conn = getConn();
             PreparedStatement pstmt = conn.prepareStatement(sql);
    ...
    public List<String> getUniqueDepartLevels() {
        ...
        String sql = "SELECT DISTINCT d.depart_level"
        		   + " FROM process p INNER JOIN department d ON p.dapart_id2 = d.dapart_id2"
        		   + " WHERE d.depart_level IS NOT NULL";
    ...
    // getDepartLevelsByItemCode - 품목 코드에 해당하는 부서 목록을 조회
    public List<String> getDepartLevelsByItemCode(String itemCode) {
        List<String> departLevels = new ArrayList<>();
        String sql = "SELECT DISTINCT d.depart_level"
                   + " FROM process p"
                   + " INNER JOIN department d ON p.dapart_id2 = d.dapart_id2"
                   + " WHERE p.item_code = ? AND d.depart_level IS NOT NULL";
    ...
    public List<String> getUniqueProcNames() {
        ...
        String sql = "SELECT DISTINCT proc_name"
        		   + " FROM process WHERE proc_name IS NOT NULL";
        try (Connection conn = getConn();
             PreparedStatement pstmt = conn.prepareStatement(sql);
             ResultSet rs = pstmt.executeQuery()) {
            while (rs.next()) {
    ...
    public List<String> getUniqueProcNamesByDepart(String departLevel) {
        List<String> procNames = new ArrayList<>();
        String sql = "SELECT DISTINCT proc_name FROM process"
        		   + " WHERE depart_level = ? AND proc_name IS NOT NULL";
        try (Connection conn = getConn();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
    ...
    public List<String> getProcNamesByItemAndDepart(String itemCode, String departLevel) {
        List<String> procNames = new ArrayList<>();
        String sql = "SELECT DISTINCT p.proc_name"
                   + " FROM process p"
                   + " INNER JOIN department d ON p.dapart_id2 = d.dapart_id2"
                   + " WHERE p.item_code = ? AND d.depart_level = ?";
        try (Connection conn = getConn();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setString(1, itemCode);
    ...
    public boolean updateProcess(ProcessDTO dto) {
        ...
        try (Connection conn = getConn();
            ...
            pstmt.setString(1, dto.getProc_name());
            pstmt.setInt(2, dto.getProcess_check());
            pstmt.setString(3, dto.getDapart_id2()); // Use dapart_id2 here
            pstmt.setString(4, dto.getProc_img());
            pstmt.setString(5, dto.getProc_info());
            pstmt.setInt(6, dto.getProc_seq());
            pstmt.setString(7, dto.getItem_code());
    ...
    private ProcessDTO createProcessDTO(ResultSet rs) throws SQLException {
        ...
        // 조인된 테이블에서 depart_level을 가져옵니다.
        // 컬럼이 존재하지 않을 경우를 대비하여 예외 처리가 필요합니다.
        try {
