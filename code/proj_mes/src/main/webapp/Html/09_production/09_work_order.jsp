...
<div class="sidenwrap">
                    ...
                    <tbody>
                        <c:forEach var="wo" items="${list}">
                            <tr class="data">
                                ...
                                <td><a href="workorder?wo_num=${wo.woNum}&amp;action=view" class="wo-link">${wo.woNum}</a></td>
                                <td>${wo.woDate}</td>
                                <td>${wo.workerID}</td>
                                <td>${wo.woDuedate}</td>
                                <td>${wo.item_code}</td>
    ...
    <div class="panel" id="panel-add">
        ...
        <form method="post" action="workorder" id="form-add" class="wrap-table">
            <!-- action: add / update -->
            <input type="hidden" id="action-input" name="action" value="${param.action == 'edit' ? 'update' : 'add'}">

            <!-- wo_num hidden -->
            <input type="hidden" name="wo_num" value="${detailWO.woNum}">

            <!-- bom_id / proc_id hidden -->
            ...
            <div class="form-group">
                <label>지시일</label>
                <input type="date" name="wo_date" value="${detailWO.woDate}">
            </div>
            <div class="form-group">
                <label>납기일</label>
            ...
            <div class="form-group">
                <label>담당자</label>
                <input type="text" name="worker_id" value="${detailWO.workerID}" placeholder="담당자명 입력">
            </div>
            <div class="form-group">
                <label>지시 수량</label>
                <input type="number" name="wo_pq" min="1" value="${detailWO.woPQ}" placeholder="지시 수량 입력">
            </div>
...
document.addEventListener('DOMContentLoaded', function(){
    const p = document.querySelector('#panel-down');
    if (p) p.classList.add('open');
});
</script>
</c:if>

<!-- editMode -> 등록/수정 패널 열기 -->
<c:if test="${not empty editMode || param.action == 'edit'}">
<script>
document.addEventListener('DOMContentLoaded', function(){
    const p = document.querySelector('#panel-add');
    if (p) p.classList.add('open');
});
</script>
</c:if>

<!-- ================= JS: radio 선택 시 hidden 값만 업데이트 ================= -->
<script>
document.addEventListener('DOMContentLoaded', function() {
    const panelAdd = document.querySelector('#panel-add');
    if (!panelAdd) return;

    const bomIdInput = panelAdd.querySelector('#hidden-bom-id');
    const procIdInput = panelAdd.querySelector('#hidden-proc-id');

    const radios = panelAdd.querySelectorAll('input[name="item_code"]');
    radios.forEach(r => {
        r.addEventListener('change', function() {
            if (bomIdInput) bomIdInput.value = this.dataset.bomId || '';
            if (procIdInput) procIdInput.value = this.dataset.procId || '';
        });
    });
});
</script>
